...
class Advent extends Command
{
    ...
    /**
     * Array of execution times in ms.
     */
    private $times = [];
    ...
    public function handle()
    {
        ...
        $this->line('All days took: '.collect($this->times)->sum().'ms');
    }
...
1529';
        ...
        foreach ($records as $level1) {
            ...
            foreach ($records as $level2) {
                if ($solved || $level1 === $level2) break;

                foreach ($records as $level3) {
                    if ($solved || $level2 === $level3) break;
...
.....#...#.###...#..#........#.';
        ...
        $this->times[] = $time;
        $this->table(['Part 2: Answer'], [
            [array_product(collect($slopes)->pluck('trees')->toArray())]
        ]);
        $this->line('took: '.$time.'ms');
    }
}
