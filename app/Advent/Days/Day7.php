...
class Day7 {
    ...
    public $title = 'Day 7: Handy Haversacks';
    ...
    private function part1()
    {
        ...
        foreach ($this->bags as $bag => $contents) {
            if ($bag === 'shiny gold') continue;

            if ($this->holdsGold($bag)) $this->valids[$bag] = $bag;
        }
    ...
    /**
     * Check recursively if a bag can contain a shiny gold bag.
     *
     * @param string $bag
     * @return boolean
     */
    public function holdsGold(string $bag): bool
    {
        if (! count($this->bags[$bag])) return false;

        if (in_array('shiny gold', $this->bags[$bag])) return true;

        foreach ($this->bags[$bag] as $inner) {
            if ($this->holdsGold($inner)) $valid = true;
        }
    ...
    /**
     * Get all the bags and the bags they can contain.
     *
     * @return void
     */
    private function setBags(): void
    {
        ...
        foreach ($lines as $line) {
            $parts = explode('bags', $line, 2);

            $color = trim($parts[0]);

            if (isset($this->bags[$color])) continue;

            if (strpos($parts[1], 'no other')) {
                $this->bags[$color] = [];
                continue;
            }
            ...
            foreach ($parts as $part) {
                preg_match("/([0-9]+)(.*)(?=bag)/", $part, $match);
                $this->bags[$color][] = trim($match[2]);
            }
        }
    }

    /**
     * Set all the bag rules.
     */
    private function setRules()
    {
        preg_match_all('#([\w ]+?) bags contain ([\w ,]+? bags?)\.#', $this->getInput(), $mo);
        $outer = $mo[1];
        $inner = array_map(
            function($bags) {
        ...
        $this->rules = array_combine($outer, $inner);
    }

    /**
     * Process the inner bags of a set of outer bags
     *
     * @param array $outers
     * @param array $rules
     * @return array
     */
    public function processInners(array $outers): array {
        $required = [];

        foreach ($outers as $outer => $outer_quantity) {
            foreach (($this->rules[$outer] ?? []) as $inner => $inner_quantity) {
                $required[$inner] = ($required[$inner] ?? 0) + $outer_quantity * $inner_quantity;
            }
        }
        return $required;
    }
    ...
    {
        return 'dim red bags contain 2 bright gold bags, 5 striped fuchsia bags.
