...
class Day14 {
    ...
    public $title = 'Day 14: Docking Data';
    ...
    private function part1()
    {
        ...
        foreach ($lines as $line) {
            if (Str::startsWith($line, 'mask')) {
                $mask = str_split(explode(' ', $line)[2]);
                $length = count($mask);
                $mask = collect($mask)->filter(fn($m) => $m != 'X')->all();
            } else {
                $address = Str::between($line, '[', ']');
                $value = explode(' ', $line)[2];
                $binary = decbin($value);
                $binary = Str::padLeft($binary, $length ?? 36, '0');
                $binary = str_split($binary);

                $merged = array_replace($binary, $mask);

                $binary = implode('', $merged);
                $memory[$address] = bindec($binary);
            }
        }

        $result = array_sum($memory);
    ...
    private function part2()
    {
        ...
        foreach ($lines as $line) {
            if (Str::startsWith($line, 'mask')) {
                ...
                foreach ($mask as $i => $val) {
                    if ($val == 1 || $val == 'X') {
                        $changes[$i] = $val;
                    }
                }

                $merged = array_replace($bits, $changes);

                $xs = collect($merged)->filter(fn($m) => $m == 'X')->all();

                $combinations = $this->getCombinations(array_keys($xs));
                $combinations[] = [];

                foreach ($combinations as $combination) {
                    $curmask = $merged;

                    foreach ($curmask as $i => $x) {
                    ...
                    $address = array_replace($merged, $curmask);
                    $addresses[] = bindec(implode('', $address));
                }
    ...
    private function getCombinations($in) {
        $count = count($in);
        $members = pow(2, $count);
        $combinations = [];
    ...
    {
        return 'mask = 111101X010011110100100110100101X0X0X
mem[37049] = 1010356
mem[5632] = 28913
mem[9384] = 7522
